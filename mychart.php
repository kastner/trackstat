...
$varx = split(" ", $db->getOne($sql));
$sql = 'select unix_timestamp(convert_tz(mark_start, "-0:00", "-4:00")) m_s, to_days(convert_tz(mark_end, "-0:00", "-4:00")) - TO_DAYS(convert_tz(mark_start, "-0:00", "-4:00")) as days, username,var,sum(value) total, date(convert_tz(added, "-0:00", "-4:00")) day from track t LEFT JOIN marks_users mu ON mu.user_id = t.user_id LEFT JOIN users u ON u.id = t.user_id LEFT JOIN marks m ON m.id = mu.mark_id where m.id = ' . $mark . ' and added >= mark_start and added <= mark_end group by t.user_id,date(convert_tz(added, "-0:00", "-4:00")), var order by var;';
$rows = $db->getAll($sql);
